...
// ========== 裝備強化 ==========
function calcEqBonus(eq){ let a=0,h=0; eq.forEach(e=>{ if(e.equipped) e.type==='weapon'?a+=e.bonus:h+=e.bonus; }); return{atk:a,hp:h}; }
function setBonus(eq){ let w=eq.filter(e=>e.type==='weapon'&&e.equipped).length, a=eq.filter(e=>e.type==='armor'&&e.equipped).length; return{atk:w>=2?0.05:0,hp:a>=2?0.05:0}; }
function applyEquipBonus(){ let b=calcEqBonus(window.equipment), s=setBonus(window.equipment); let atkBonus = window.baseAtk + b.atk*(1+s.atk); let hpBonus = window.baseHp + b.hp*(1+s.hp); if(window.petAtkBonus) atkBonus *= (1+window.petAtkBonus); if(window.petHpBonus) hpBonus *= (1+window.petHpBonus); window.atk = Math.floor(atkBonus); window.maxHp = Math.floor(hpBonus); if(window.hp>window.maxHp) window.hp=window.maxHp; updateUI(); }
...
function generateEquipment(){ let t=rand(0,1)?'weapon':'armor', q=randomQuality(), b=rand(q.min,q.max); return { id:Date.now()+'-'+Math.random(), type:t, quality:q.name, color:q.color, bonus:b, name:`${q.name} ${t==='weapon'?'武器':'防具'}`, equipped:false, level:0 }; }
function showBackpack(){
  ...
  let cls=document.createElement('button'); cls.innerText='關閉'; cls.onclick=()=>ov.remove(); cd.appendChild(cls); ov.appendChild(cd); document.body.appendChild(ov);
}
function toggleEquip(id){
  let item = window.equipment.find(e=>e.id==id);
  if(!item) return;
  if(item.equipped){
    item.equipped=false; toast(`已卸下 ${item.name}`);
  } else {
    let same = window.equipment.find(e=>e.type===item.type&&e.equipped);
    if(same) same.equipped=false;
    item.equipped=true; toast(`已裝備 ${item.name}`);
  }
  applyEquipBonus(); saveGame(); showBackpack();
}
...
function refreshMonster(){ window.curMonster = genMonster(); window.restUsed=false; updateUI(); log(`✨ ${window.curMonster.isBoss?'👑BOSS ':''}${window.curMonster.emoji}${window.curMonster.name} Lv.${window.curMonster.mlv} | HP${window.curMonster.hp} 攻${window.curMonster.atk}`); if(window.curMonster.isBoss) bossSound(); }
function updateUI(){
...
window.googleLogin = async () => { try{ let r=await auth.signInWithPopup(provider); currentUser=r.user; let btn=document.getElementById('googleBtn'); if(btn){ btn.innerText=`👤 ${currentUser.displayName}`; btn.disabled=true; } toast(`歡迎 ${currentUser.displayName}`); await loadCloudSave(); } catch(e){ toast('登入失敗',1); } };
window.googleLogout = async () => { await auth.signOut(); currentUser=null; let btn=document.getElementById('googleBtn'); if(btn){ btn.innerText='登入'; btn.disabled=false; } toast('已登出'); loadGame(); };
auth.onAuthStateChanged(u => { currentUser=u; let btn=document.getElementById('googleBtn'); if(btn){ if(u){ btn.innerText=`👤 ${u.displayName}`; btn.disabled=true; } else { btn.innerText='登入'; btn.disabled=false; } } });
function saveGame(){
  const state = { lv:window.lv, exp:window.exp, baseAtk:window.baseAtk, baseHp:window.baseHp, hp:window.hp, maxHp:window.maxHp, atk:window.atk, restUsed:window.restUsed, difficulty:window.difficulty, equipment:window.equipment, deathStreak:window.deathStreak, lastDmg:window.lastDmg, totalKills:window.totalKills, maxDamage:window.maxDamage, maxLevel:window.maxLevel, gold:window.gold, ownedPets, currentPet:currentPet?currentPet.id:null, talentPoints, talents:talents.map(t=>({id:t.id, current:t.current})), dailyQuests };
  localStorage.setItem('gameSave', JSON.stringify(state)); toast('💾 存檔');
...
function defeatMonster(){
  ...
  if(Math.random()<0.3 || window.curMonster.isBoss){
    ...
    if(!window.equipment.some(e=>e.type===eq.type&&e.equipped)){
      eq.equipped = true; applyEquipBonus(); toast(`✨自動裝備`);
    }
    saveGame();
  }
  levelUp(); refreshMonster(); updateUI(); saveGame(); defeatSound();
}
function attack(){
  ...
  if(window.curMonster.hp <= 0) defeatMonster(); else monsterCounter();
  updateUI(); saveGame();
}
