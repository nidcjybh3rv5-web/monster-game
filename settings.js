window.renderSettings = () => {
  ...
  c.innerHTML = `
    ...
    <div class="setting-item"><div><div class="setting-label">🔔 音效開關</div><div class="setting-desc">開啟/關閉</div></div><label class="switch"><input type="checkbox" id="sfxCheck" ${localStorage.mute !== '1' ? 'checked' : ''}><span class="slider"></span></label></div>
    <div class="setting-item"><div><div class="setting-label">⚔️ 難度</div><div class="setting-desc">影響強度</div></div><select id="diffSelect" class="difficulty-select"><option value="easy">簡單</option><option value="normal" selected>普通</option><option value="hard">困難</option><option value="extreme">極限</option></select></div>
